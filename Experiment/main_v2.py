...
def object_detection(stream):
    #function for detecting the RC car
    global car_detected
    ...
    if cap.isOpened():
        ...
        while True:
            ...
            for contour in contours:
                ...
                if area > 500: #adjust the threshold if needed
                    car_detected = True
...
async def run():
    ...
    detect_thread.start() #start object detection thread
    ...
    # await drone.offboard.set_velocity_ned(VelocityNedYaw(0.1,0.1,0.1,0.1))
    # await asyncio.sleep(2)

    # await drone.offboard.set_velocity_body(VelocityBodyYawspeed(0.1,0.1,0.1,0.1))
    # await asyncio.sleep(2)
        ...
        if not car_detected:
            try:
                if policy[i] == policy[i+1]:
                    current_cell= get_next_location(current_cell, action, environment_columns, environment_rows)
                    continue
                else:
                    ...
                    if next_cell in constraint_locations:#go a bit down to monitor the constraint locations
                        await drone.offboard.set_position_velocity_ned(PositionNedYaw(round(next_cell[0]*0.6, 1)*-1, round(next_cell[1]*0.6, 1), -1, 90.0),VelocityNedYaw(0.1,0.1,0,0))
                        await asyncio.sleep(4)
                        await drone.offboard.set_position_velocity_ned(PositionNedYaw(round(next_cell[0]*0.6, 1)*-1, round(next_cell[1]*0.6, 1), -0.6, 90.0),VelocityNedYaw(0.1,0.1,0,0))
                        await asyncio.sleep(4)
                    else:
                        await drone.offboard.set_position_velocity_ned(PositionNedYaw(round(next_cell[0]*0.6, 1)*-1, round(next_cell[1]*0.6, 1), -1, 90.0),VelocityNedYaw(0.1,0.1,0,0))
                        await asyncio.sleep(2)
            ...
            while current_cell not in safe_regions:
                ...
                next_cell= get_next_location(current_cell, action, environment_columns, environment_rows)
                await drone.offboard.set_position_velocity_ned(PositionNedYaw(round(next_cell[0]*0.6, 1)*-1, round(next_cell[1]*0.6, 1), -1, 90.0))
                current_cell = next_cell
                print(current_cell)
                await asyncio.sleep(2)
            break
